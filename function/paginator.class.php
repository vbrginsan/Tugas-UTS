...
class Paginator {
    ...
    public function createLinks1( $links, $file ) {
        $this->dt = $file;
        if ( $this->batas == 'all' ) {
            return '';
        }
        ...
        $class      = ( $this->halaman == 1 ) ? "disabled" : "";
        $html       .= '<li class="' . $class . ' symbol"><a href="?limit=' . $this->batas . '&a='.$this->dt.'&page=1">&laquo;</a></li>';
        $html       .= '<li class="' . $class . ' symbol"><a href="?limit=' . $this->batas . '&a='.$this->dt.'&page=' . ( $this->halaman - 1 ) . '">&lsaquo;</a></li>';
    
        if ( $start > 1 ) {
            $html   .= '<li><a href="?limit=' . $this->batas . '&a='.$this->dt.'&page=1">1</a></li>';
            $html   .= '<li><span class="titik">...</span></li>';
        }
    
        for ( $i = $start ; $i <= $end; $i++ ) {
            $class  = ( $this->halaman == $i ) ? "active" : "";
            $html   .= '<li class="' . $class . '"><a href="?limit=' . $this->batas . '&a='.$this->dt.'&page=' . $i . '">' . $i . '</a></li>';
        }
    
        if ( $end < $last ) {
            $html   .= '<li class="disabled"><span>...</span></li>';
            $html   .= '<li><a href="?limit=' . $this->batas . '&a='.$this->dt.'&page=' . $last . '">' . $last . '</a></li>';
        }
    
        $class      = ( $this->halaman == $last ) ? "disabled" : "";
        $html       .= '<li class="' . $class . ' symbol"><a href="?limit=' . $this->batas . '&a='.$this->dt.'&page=' . ( $this->halaman + 1 ) . '">&rsaquo;</a></li>';
        $html       .= '<li class="' . $class . ' symbol"><a href="?limit=' . $this->batas . '&a='.$this->dt.'&page=' . ( $last ) . '">&raquo;</a></li>';
